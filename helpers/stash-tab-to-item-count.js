...
const stashTabToItemCount = stashTabItems => {
  ...
  const itemCount = secondary.map(itemType => ({items: getItemsWithBaseFromList(itemsWithCorrectProperties, itemType.items), itemClass: itemType.itemClass})).map(t => ({...t, count: t.items.length}))
  ...
  return { itemCount, ringsInStash, amuletsInStash };
};
